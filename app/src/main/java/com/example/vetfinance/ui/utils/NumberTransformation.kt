package com.example.vetfinance.ui.utils
...
private val integerFormatter = ThreadLocal.withInitial { DecimalFormat("#,###") }
private val currencyFormatter = ThreadLocal.withInitial { DecimalFormat("#,###.##") }
...
class NumberTransformation : VisualTransformation {
    override fun filter(text: AnnotatedString): TransformedText {
        ...
        val formattedText = if (originalText.isNotEmpty()) {
            ...
            if (number != null) {
                integerFormatter.get()!!.format(number)
            } else {
                originalText
            }
        } else {
...
fun formatCurrency(value: Double): String {
    return currencyFormatter.get()!!.format(value)
}
